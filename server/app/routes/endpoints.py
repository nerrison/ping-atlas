...
group_router = APIRouter(
    prefix="/groups/{group_id}/endpoints",
    tags=["endpoints"]
)
...
@endpoint_router.get("/{endpoint_id}")
def get_endpoint(
    endpoint_id: UUID,
    service: EndpointService = Depends(get_endpoint_service),
):
...
def update_endpoint(
    ...
    try:
        return service.update_endpoint(payload, endpoint_id)
    except ValueError as e:
        raise HTTPException(
            status_code=404,
            detail=str(e)
...
@endpoint_router.patch("/{endpoint_id}")
def patch_endpoint(
    endpoint_id: UUID,
    ...
    try:
        return service.patch_endpoint(payload, endpoint_id)
    except ValueError as e:
        raise HTTPException(
            status_code=404,
            detail=str(e)
